...
- **Analiza nota**: `ReferencePitchHz` (440 — A4, referinta scala cromatica), `InTuneToleranceCents` (2.0)
...
- Proprietati bindabile: `NoteText`, `OctaveText`, `LatinNoteText`, `NoteSeparatorText`, `FrequencyText`, `CentsText`, `NoteColor`, `FrequencyColor`, `NeedleAngle`
- `LatinNoteText` / `NoteSeparatorText`: nota in sistem latin (solfegiu) afisata alaturi de cea anglo-saxona, ex. `E2/Mi2`. `ToLatinName` mapeaza prin `LatinNoteNames` (C→Do, C#→Do#, D→Re, ... B→Si). `NoteSeparatorText` = `"/"` cand exista semnal, gol cand nu (ca sa nu apara `"—/"`)
- `NoteColor` / `FrequencyColor`: culoarea textului nota+frecventa, interpolata continuu (degrade) dupa magnitudinea centilor in `UpdateDisplayColors` → `GradientColorFor`. Tabel de ancore `ColorStops` (centi → culoare): `0`→verde `#4FC04F`, `8`→galben `#F2A640`, `25`→rosu `#D83A2E`, `50`→rosu inchis `#6E0F0F`; `Lerp` amesteca liniar R/G/B intre ancorele vecine. `FrequencyColor` = `Dim(NoteColor, 0.9)` (nuanta usor mai inchisa). Cand nu exista semnal revin la `IdleColor` (`#F2A640`)
...
- Afisaj pe 3 coloane (`Grid ColumnDefinitions="*,2*,*"`, `LayoutBounds="0.5,0.73,0.85,0.12"`) plasat peste caseta de afisaj care e deja desenata in imaginea de fundal. Coloana centrala e de 2x latimea laterelelor (raport 1:2:1) ca sa coincida cu caseta neagra; coloanele laterale au `Margin="0,6,0,0"`:
  ...
  - **Centru**: **doar text** (fara niciun chenar desenat) plasat peste caseta neagra din imagine — nota in ambele sisteme pe un rand `anglo-saxon/latin` cu octavele ca subscript (`FormattedString` cu 5 `Span`-uri: `NoteText` + `OctaveText` + `NoteSeparatorText` + `LatinNoteText` + `OctaveText`, fonturi 20 / 10, ex. `E2/Mi2`) si frecventa dedesubt (`TextColor` legat de `FrequencyColor`, font 12). Toate span-urile notei au `TextColor` legat de `NoteColor`; culorile curg in degrade dupa abatere (verde → galben → rosu → rosu inchis)
...
| `ViewModels/TunerViewModel.cs` | Modificat (proprietati NoteText/OctaveText/LatinNoteText/NoteSeparatorText/CentsText pentru afisaj pe coloane; nume nota in sistem latin via `LatinNoteNames`; `NoteColor`/`FrequencyColor` in degrade dupa centi; smoothing-ul pe frecventa mutat in `FrequencyStabilizer`; deadband pe cents + EMA pe unghi; unghi ac ±50° pentru ±50 centi; eliminat butonul — metode `StartAsync`/`StopAsync` in loc de `ToggleListeningCommand`) |
...
| `MainPage.xaml` | Modificat (afisaj pe 3 coloane `*,2*,*` la `0.5,0.73`; doar text peste caseta neagra din imagine, fara `Border`; nota pe ambele sisteme `E2/Mi2` din 5 `Span`-uri fonturi 20/10; `TextColor` nota/frecventa legat de `NoteColor`/`FrequencyColor`; etichete laterale culoare `#C9B68A`; eliminat butonul Start/Stop) |
...
- **Nume nota in doua sisteme (anglo-saxon + latin)**: afisajul arata nota in ambele notatii pe un rand, ex. `E2/Mi2`. `NoteAnalyzerService` ramane neschimbat (produce doar numele anglo-saxon `C..B`); conversia la latin (solfegiu) e pur prezentare, in `TunerViewModel.ToLatinName` via dictionarul `LatinNoteNames`. `NoteSeparatorText` tine separatorul `/` doar cand exista semnal (gol altfel, ca afisajul fara nota sa fie doar `—`, nu `—/`). In XAML, randul notei e un `FormattedString` cu 5 span-uri (nota EN + octava sub + `/` + nota latina + octava sub), toate cu `TextColor` legat de `NoteColor`. Fontul a fost redus la 20/10 (de la 34/16) ca ambele notatii sa incapa in fereastra ingusta de 249px, inclusiv pentru note cu diez (`C#2/Do#2`). Pentru a regla maparea (sau a adauga alte sisteme) se editeaza `LatinNoteNames`.
- **Culoare text afisaj in degrade dupa abatere**: textul nota+frecventa isi schimba culoarea CONTINUU (nu pe zone discrete) dupa magnitudinea centilor, in `TunerViewModel.UpdateDisplayColors` → `GradientColorFor`, expusa prin `NoteColor`/`FrequencyColor` (View-ul doar leaga `TextColor`, nu contine logica). Degradeul e definit de tabelul `ColorStops` (pereche centi→culoare): `0`→verde `#4FC04F`, `8`→galben `#F2A640`, `25`→rosu `#D83A2E`, `50`→rosu inchis `#6E0F0F`. `GradientColorFor` clampeaza magnitudinea la [0, 50], gaseste perechea de ancore vecine si interpoleaza liniar (`Lerp` pe R/G/B). `FrequencyColor` e o versiune usor mai inchisa (`Dim(NoteColor, 0.9)`) ca sa pastreze nuanta istorica nota>frecventa. Cand nu exista semnal, ambele revin la `IdleColor` (`#F2A640`). Tabelul de culori e UI pur, deci sta in ViewModel (nu in `TunerSettings`); pentru a regla degradeul se editeaza ancorele din `ColorStops`.
